    ...
    socketInstance.on('new_message', (data) => {
      ...
      if (data.session_id === selectedSessionIdRef.current) {
        if (data.message.role === 'user') {
          ...
          // Assistant message received - hide typing animation
          console.log(
            'Assistant message received via Socket.IO - hiding typing animation'
          )
          setIncomingMessageLoading(false)
        }
      }

      // Update conversations list immediately for real-time updates
      queryClient.setQueryData(['conversations', apiKey], (oldData: any) => {
        if (!oldData) return oldData

        // Find and update the conversation with new message
        const updatedConversations = oldData.map((conv: any) => {
          if (conv.session_id === data.session_id) {
            return {
              ...conv,
      ...
      if (data.session_id === selectedSessionIdRef.current) {
        console.log('Updating messages for current session immediately')
        queryClient.setQueryData(
          ['messages', selectedSessionIdRef.current, apiKey],
          (oldData: any) => {
            if (!oldData) return oldData

            const newMessage = {
              _id: `socket_${Date.now()}_${Math.random()}`,
              id: `socket_${Date.now()}_${Math.random()}`,
              organization_id: data.organization_id,
              visitor_id: '', // Will be filled by backend
              session_id: data.session_id,
              role: data.message.role,
              content: data.message.content,
              created_at: data.message.timestamp,
              metadata: data.message.agent_id
                ? { agent_id: data.message.agent_id }
                : {},
            }

            return {
              ...oldData,
              conversations: [...oldData.conversations, newMessage],
            }
          }
        )
      } else {
        console.log(
          'Message is for different session, not updating current messages'
        )
      }
    })
