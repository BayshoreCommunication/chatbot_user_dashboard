...
export default function ApiDebug() {
  ...
  return (
    <div className="fixed bottom-4 right-4 bg-blue-100 border border-blue-300 rounded p-3 text-xs max-w-sm z-50">
      <h4 className="font-bold mb-2">API Debug (Dev Only)</h4>
      <div className="space-y-1">
        ...
        <div>
          ...
          <span className={`ml-1 ${apiStatus.healthz === 'success' ? 'text-green-600' : apiStatus.healthz === 'failed' ? 'text-red-600' : 'text-yellow-600'}`}>
            {apiStatus.healthz}
          </span>
        </div>
        <div>
          <strong>Instant Reply:</strong> 
          <span className={`ml-1 ${apiStatus.instantReply === 'success' ? 'text-green-600' : apiStatus.instantReply === 'failed' ? 'text-red-600' : 'text-yellow-600'}`}>
            {apiStatus.instantReply}
          </span>
        </div>
        <div>
          <strong>FAQ:</strong> 
          <span className={`ml-1 ${apiStatus.faq === 'success' ? 'text-green-600' : apiStatus.faq === 'failed' ? 'text-red-600' : 'text-yellow-600'}`}>
            {apiStatus.faq}
          </span>
        </div>
        <div>
          <strong>Training:</strong> 
          <span className={`ml-1 ${apiStatus.training === 'success' ? 'text-green-600' : apiStatus.training === 'failed' ? 'text-red-600' : 'text-yellow-600'}`}>
            {apiStatus.training}
          </span>
        </div>
      </div>
